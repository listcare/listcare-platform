...
// Keep the app buildable before the owner adds the OpenAI key in hosting.
// Tool requests return a clear setup message until the key is configured.
...
export async function POST(req:NextRequest){
  try{
    if(!openai)throw new Error("Το εργαλείο AI δεν έχει συνδεθεί ακόμη. Πρόσθεσε το OpenAI API key στις ρυθμίσεις.");
